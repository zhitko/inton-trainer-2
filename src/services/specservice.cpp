...
std::vector<std::vector<double>> SpecService::getSpec(
    ...
    // Validate FFT length - WORLD/CheapTrick typically requires minimum 1024 for audio
    if (fftLength < 1024) {
        LOG_WARNING() << "FFT length " << fftLength << " may be too small for reliable spectrum extraction. "
                      << "Recommended minimum is 1024 for " << sampleRate << " Hz sample rate. Attempting anyway...";
    }

    // Create SpectrumExtraction instance using WORLD algorithm (CheapTrick)
    sptk::SpectrumExtraction specExtractor(
        fftLength,
        frameShift,
        sampleRate,
        f0Refinement,
    ...
    if (!specExtractor.IsValid()) {
        LOG_CRITICAL() << "SpectrumExtraction initialization failed - FFT length: " << fftLength
                       << ", Frame shift: " << frameShift
                       << ", Sample rate: " << sampleRate;
        LOG_CRITICAL() << "Check if FFT length is appropriate for the sample rate (try 1024, 2048, or 4096)";
        return result;
    }
    ...
    if (!specExtractor.Run(inputWaveData, f0, &result)) {
        ...
        LOG_CRITICAL() << "Input size: " << inputWaveData.size() << ", F0 frames: " << f0.size();
        
        // Check for WORLD/CheapTrick constraints
        double minSupportedF0 = 2.0 * sampleRate / fftLength;
        LOG_CRITICAL() << "Note: WORLD algorithm requires F0 > 2.0 * sampleRate / fftLength (" << minSupportedF0 << " Hz)";
        LOG_CRITICAL() << "Your current settings might produce F0 values below this threshold. Try increasing FFT length or minF0.";
        return result;
    }
