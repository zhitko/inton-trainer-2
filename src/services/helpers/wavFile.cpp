...
WaveFile* waveOpenHFile(int handle)
{
    ...
    if (waveFile == nullptr) {
        LOG_CRITICAL() << "[waveOpenHFile] Memory Allocation Error: Could not "
    ...
    if (waveFile->file == nullptr) {
        LOG_CRITICAL() << "[waveOpenHFile] Could not open input file "
                       << (waveFile->filePath ? waveFile->filePath : "null");
        waveCloseFile(waveFile);
        LOG_DEBUG() << "Finish: waveOpenHFile - result=nullptr (file open failed)";
        return nullptr;
    }
    WaveFile* result = processFile(waveFile);
    LOG_DEBUG() << "Finish: waveOpenHFile - result="
...
WaveFile* waveOpenFile(const std::string& path)
{
    ...
    WaveFile* waveFile = initWaveFile();
    if (waveFile == nullptr) {
        LOG_CRITICAL() << "[waveOpenFile] Memory Allocation Error: Could not "
                          "allocate memory for Wave File";
    ...
    try {
        ...
        LOG_CRITICAL() << "[waveOpenFile] Error converting path: " << e.what();
        waveFile->file = nullptr;
    }
#else
    ...
    if (waveFile->file == nullptr) {
        LOG_CRITICAL() << "[waveOpenFile] Could not open input file "
                       << (waveFile->filePath ? waveFile->filePath : "null");
        waveCloseFile(waveFile);
    ...
    LOG_DEBUG() << "Finish: waveOpenFile - result="
                << (waveFile ? "success" : "failed");
    return waveFile;
}
WaveFile* processFile(WaveFile* waveFile)
{
    ...
    if (waveFile->waveHeader == nullptr) {
        LOG_CRITICAL() << "[processFile] Memory Allocation Error: Could not "
                          "allocate memory for Wave File Header";
        waveCloseFile(waveFile);
        return nullptr;
    }
    fread(waveFile->waveHeader, sizeof(WaveHeader), 1, waveFile->file);
    if (ferror(waveFile->file) != 0) {
        LOG_CRITICAL() << "[processFile] Error reading input file "
                       << (waveFile->filePath ? waveFile->filePath : "null");
        waveCloseFile(waveFile);
        return nullptr;
    }
    ...
    if (strncmp(&(waveFile->waveHeader->chunkID[0]), "RIFF", 4) != 0) {
        LOG_WARNING() << "[processFile] Input file is not a RIFF file";
        waveCloseFile(waveFile);
        return nullptr;
    }

    if (strncmp(&(waveFile->waveHeader->riffType[0]), "WAVE", 4) != 0) {
        LOG_WARNING() << "[processFile] Input file is not a WAVE file";
        waveCloseFile(waveFile);
        return nullptr;
    }

    uint32_t remainingFileSize = littleEndianBytesToUInt32(waveFile->waveHeader->dataSize);
    int size = remainingFileSize - sizeof(waveFile->waveHeader->riffType); // dataSize does not counf the chunkID or the
                                                                           // dataSize, so remove the riffType size to get
                                                                           // the length of the rest of the file.

    if (size <= 0) {
        LOG_WARNING() << "[processFile] Input file is an empty WAVE file";
        waveCloseFile(waveFile);
        return nullptr;
    }
    ...
    while (1) {
        ...
        if (strncmp(&nextChunkID[0], "fmt ", 4) == 0) {
            ...
            if (littleEndianBytesToUInt16(waveFile->formatChunk->compressionCode) != (uint16_t)1) {
                LOG_WARNING()
                    << "[processFile] Compressed audio formats are not supported";
                waveCloseFile(waveFile);
                return nullptr;
            }

            // Note: For compressed audio data there may be extra bytes appended to
            // the format chunk, but as we are only handling uncompressed data we
            // shouldn't encounter them

            // There may or may not be extra data at the end of the fomat chunk.  For
            // uncompressed audio there should be no need, but some files may still
            // have it. if formatChunk.chunkDataSize > 16 (16 = the number of bytes
            // for the format chunk, not counting the 4 byte ID and the chunkDataSize
            // itself) there is extra data
            ...
            uint32ToLittleEndianBytes(sampleDataSize,
                waveFile->dataChunk->chunkDataSize);

            // TODO: check if no errors in allocate and read data
            waveFile->dataChunk->waveformData = (char*)malloc(sampleDataSize);
            fread(waveFile->dataChunk->waveformData, sampleDataSize, 1,
                waveFile->file);
            if (ferror(waveFile->file) != 0) {
                LOG_CRITICAL() << "[processFile] Error reading input file "
                               << (waveFile->filePath ? waveFile->filePath : "null");
            ...
            // Skip to the end of the chunk.  Chunks must be aligned to 2 byte
            // boundaries, but any padding at the end of a chunk is not included in
            // the chunkDataSize
            ...
            LOG_DEBUG() << "[processFile] Found CUE chunk";
            ...
            for (uint32_t cuePointIndex = 0; cuePointIndex < cuePointsCount;
                 cuePointIndex++) {
                CuePoint* cue_point = &existingCuePoints[cuePointIndex];
                LOG_DEBUG() << "Found CUE POINT " << cuePointIndex;
                ...
                LOG_DEBUG() << "[processFile] Read CUE POINT cuePointID "
                            << INT32(cue_point->cuePointID);
                LOG_DEBUG() << "[processFile] Read CUE POINT playOrderPosition "
                            << INT32(cue_point->playOrderPosition);
                LOG_DEBUG() << "[processFile] Read CUE POINT dataChunkID "
                            << cue_point->dataChunkID[0] << cue_point->dataChunkID[1]
                            << cue_point->dataChunkID[2] << cue_point->dataChunkID[3];
                LOG_DEBUG() << "[processFile] Read CUE POINT chunkStart "
                            << INT32(cue_point->chunkStart);
                LOG_DEBUG() << "[processFile] Read CUE POINT blockStart "
                            << INT32(cue_point->blockStart);
                LOG_DEBUG() << "[processFile] Read CUE POINT frameOffset "
            ...
            if (waveFile->dataChunk == nullptr) {
                LOG_CRITICAL() << "[processFile] Memory Allocation Error: Could not "
                                  "allocate memory for Wave File Cue Chunk";
                waveCloseFile(waveFile);
                free(existingCuePoints);
                return nullptr;
            }
            ...
            uint32ToLittleEndianBytes(cueChunkDataSize,
                waveFile->cueChunk->chunkDataSize);
            uint32ToLittleEndianBytes(cuePointsCount,
                waveFile->cueChunk->cuePointsCount);
            waveFile->cueChunk->cuePoints = existingCuePoints;

            LOG_DEBUG() << "[processFile] Read CUE chunkID "
                        << waveFile->cueChunk->chunkID[0]
                        << waveFile->cueChunk->chunkID[1]
                        << waveFile->cueChunk->chunkID[2]
                        << waveFile->cueChunk->chunkID[3];
            LOG_DEBUG() << "[processFile] Read CUE chunkDataSize "
                        << INT32(waveFile->cueChunk->chunkDataSize);
            LOG_DEBUG() << "[processFile] Read CUE cuePointsCount "
                        << INT32(waveFile->cueChunk->cuePointsCount);
        } else if (strncmp(&nextChunkID[0], "LIST", 4) == 0) {
            LOG_DEBUG() << "[processFile] Found LIST chunk";
            // We found an existing List Chunk
            waveFile->listChunks = (ListChunk*)realloc(
                waveFile->listChunks, sizeof(ListChunk) * (waveFile->listCount + 1));
            ...
            LOG_DEBUG() << "[processFile] Read LIST chunkID " << listChunk->chunkID[0]
                        << listChunk->chunkID[1] << listChunk->chunkID[2]
                        << listChunk->chunkID[3];
            LOG_DEBUG() << "[processFile] Read LIST chunkDataSize "
                        << INT32(listChunk->chunkDataSize);
            LOG_DEBUG() << "[processFile] Read LIST typeID " << listChunk->typeID[0]
                        << listChunk->typeID[1] << listChunk->typeID[2]
                        << listChunk->typeID[3];
        } else if (strncmp(&nextChunkID[0], "ltxt", 4) == 0) {
            LOG_DEBUG() << "[processFile] Found LTXT chunk";
            ListChunk* listChunk = &(waveFile->listChunks[waveFile->listCount - 1]);
            LOG_DEBUG() << "[processFile] listCount " << waveFile->listCount;

            // We found an existing LTXT chunk in LIST chunk
            listChunk->ltxtChunks = (LtxtChunk*)realloc(listChunk->ltxtChunks,
                sizeof(LtxtChunk) * (listChunk->ltxtCount + 1));
            ...
            if (ltxtChunk == nullptr) {
                LOG_CRITICAL() << "[processFile] Memory Allocation Error: Could not "
                                  "allocate memory for Wave File LABL chunk";
                waveCloseFile(waveFile);
                return nullptr;
            }
            ...
            LOG_DEBUG() << "[processFile] LTXT dataSize " << ltxtChunkDataSize;
            ...
            if (ltxtChunkDataSize > 20) {
                ...
                LOG_DEBUG() << "[processFile] LTXT text " << ltxtChunk->text;
            } else {
                ltxtChunk->text = nullptr;

                LOG_DEBUG() << "[processFile] LTXT text <skip>";
            }
            LOG_DEBUG() << "[processFile] Read LTXT chunkID " << ltxtChunk->chunkID[0]
                        << ltxtChunk->chunkID[1] << ltxtChunk->chunkID[2]
                        << ltxtChunk->chunkID[3];
            LOG_DEBUG() << "[processFile] Read LTXT chunkDataSize "
                        << INT32(ltxtChunk->chunkDataSize);
            LOG_DEBUG() << "[processFile] Read LTXT cuePointID "
                        << INT32(ltxtChunk->cuePointID);
            LOG_DEBUG() << "[processFile] Read LTXT sampleLength "
                        << INT32(ltxtChunk->sampleLength);
            LOG_DEBUG() << "[processFile] Read LTXT purposeID "
                        << ltxtChunk->purposeID[0] << ltxtChunk->purposeID[1]
                        << ltxtChunk->purposeID[2] << ltxtChunk->purposeID[3];
            LOG_DEBUG() << "[processFile] Read LTXT country "
                        << INT16(ltxtChunk->country);
            LOG_DEBUG() << "[processFile] Read LTXT language "
                        << INT16(ltxtChunk->language);
            LOG_DEBUG() << "[processFile] Read LTXT dialect "
                        << INT16(ltxtChunk->dialect);
            LOG_DEBUG() << "[processFile] Read LTXT codePage "
                        << INT16(ltxtChunk->codePage);
        } else if (strncmp(&nextChunkID[0], "labl", 4) == 0) {
            LOG_DEBUG() << "[processFile] Found LABL chunk";
            ListChunk* listChunk = &waveFile->listChunks[waveFile->listCount - 1];

            // We found an existing LABL chunk in LIST chunk
            listChunk->lablChunks = (LablChunk*)realloc(listChunk->lablChunks,
                sizeof(LablChunk) * (listChunk->lablCount + 1));
            ...
            LOG_DEBUG() << "[processFile] LABL dataSize " << lablChunkDataSize;

            lablChunkDataSize = to_odd(lablChunkDataSize);

            if (lablChunkDataSize > 4) {
                lablChunk->text = (char*)malloc(lablChunkDataSize - 4);
                fread(lablChunk->text, sizeof(char), lablChunkDataSize - 4,
                    waveFile->file);

                LOG_DEBUG() << "[processFile] LABL text " << lablChunk->text;
            } else {
                lablChunk->text = nullptr;

                LOG_DEBUG() << "[processFile] LABL text <skip>";
            }
            LOG_DEBUG() << "[processFile] Read LABL chunkID " << lablChunk->chunkID[0]
                        << lablChunk->chunkID[1] << lablChunk->chunkID[2]
                        << lablChunk->chunkID[3];
            LOG_DEBUG() << "[processFile] Read LABL chunkDataSize "
                        << INT32(lablChunk->chunkDataSize);
            LOG_DEBUG() << "[processFile] Read LABL cuePointID "
                        << INT32(lablChunk->cuePointID);
        } else {
            LOG_WARNING() << "[processFile] Found unsuppoted chunk " << nextChunkID;
...
void waveCloseFile(WaveFile* waveFile)
{
    ...
    if (waveFile->cueChunk != nullptr)
        free(waveFile->cueChunk);
    if (waveFile->listChunks != nullptr) {
        free(waveFile->listChunks);
    }
    if (waveFile->filePath != nullptr)
        free(waveFile->filePath);
    free(waveFile);
...
WaveFile* waveCloneFile(WaveFile* waveFile)
{
    ...
    clone->waveHeader = makeWaveHeader(INT32(waveFile->waveHeader->dataSize));

    clone->formatChunk = makeFormatChunk(INT16(waveFile->formatChunk->numberOfChannels),
    ...
    memcpy(data, waveFile->dataChunk->waveformData,
        INT32(waveFile->dataChunk->chunkDataSize));
    clone->dataChunk = makeDataChunk(INT32(waveFile->dataChunk->chunkDataSize), data);

    CuePoint* cuePoints = (CuePoint*)malloc(
        sizeof(CuePoint) * INT32(waveFile->cueChunk->cuePointsCount));
    for (int i = 0; i < INT32(waveFile->cueChunk->cuePointsCount); i++) {
        cuePoints[i] = *makeCuePoint(INT32(waveFile->cueChunk->cuePoints[i].cuePointID),
    ...
    clone->cueChunk = makeCueChunk(INT32(waveFile->cueChunk->cuePointsCount), cuePoints);

    LtxtChunk* ltxtChunks = (LtxtChunk*)malloc(
        sizeof(LtxtChunk) * INT32(waveFile->cueChunk->cuePointsCount));
    LablChunk* lablChunks = (LablChunk*)malloc(
        sizeof(LablChunk) * INT32(waveFile->cueChunk->cuePointsCount));
    for (int i = 0; i < INT32(waveFile->cueChunk->cuePointsCount); i++) {
        ...
        lablChunks[i] = *makeLablChunk(INT32(waveFile->listChunks[0].lablChunks[i].cuePointID),
            waveFile->listChunks[0].lablChunks[i].text);
    }
    clone->listCount = 1;
    clone->listChunks = makeListChunk(INT32(waveFile->cueChunk->cuePointsCount), ltxtChunks,
        INT32(waveFile->cueChunk->cuePointsCount), lablChunks);
    ...
    uint16_t significantBitsPerSample)
{
    ...
    format->chunkID[2] = 't';
    format->chunkID[3] = ' ';
    // Unsigned 4-byte little endian int: Byte count for the remainder of the
    // chunk: 16 + extraFormatbytes.
    ...
    // Unsigned 2-byte little endian int: The number of bytes per sample slice.
    // This value is not affected by the number of channels and can be calculated
    // with the formula: blockAlign = significantBitsPerSample / 8 *
    // numberOfChannels
    uint16_t blockAlign = significantBitsPerSample / 8 * numberOfChannels;
    uint16ToLittleEndianBytes(blockAlign, format->blockAlign);
    // Unsigned 4-byte little endian int: This value indicates how many bytes of
    // wave data must be streamed to a D/A converter per second in order to play
    // the wave file. This information is useful when determining if data can be
    // streamed from the source fast enough to keep up with playback. = SampleRate
    // * BlockAlign.
    uint32ToLittleEndianBytes(sampleRate * blockAlign,
        format->averageBytesPerSecond);
    // Unsigned 2-byte little endian int
    uint16ToLittleEndianBytes(significantBitsPerSample,
        format->significantBitsPerSample);
    return format;
}
    ...
    uint16_t codePage, char* text)
{
    ...
    if (country != 0)
        ...
        uint16ToLittleEndianBytes(0, chunk->country);
    if (language != 0)
        uint16ToLittleEndianBytes(language, chunk->language);
    else
        uint16ToLittleEndianBytes(0, chunk->language);
    if (dialect != 0)
        uint16ToLittleEndianBytes(dialect, chunk->dialect);
    else
        uint16ToLittleEndianBytes(0, chunk->dialect);
    if (codePage != 0)
        uint16ToLittleEndianBytes(codePage, chunk->codePage);
    else
        uint16ToLittleEndianBytes(0, chunk->codePage);
...
WaveFile* makeWaveFileFromRawData(char* waveformData, uint32_t waveformDataSize,
    ...
    uint32_t* pointsLenght, char** pointsLabels)
{
    ...
    if (pointsCount > 0) {
        ...
        for (uint32_t i = 0; i < pointsCount; i++) {
            uint32_t pintId = i + 1;
            CuePoint* cuePoint = makeCuePoint(pintId, pointsOffset[i], 0, 0, pointsOffset[i]);
            cuePoints[i] = *cuePoint;
            free(cuePoint);

            LtxtChunk* ltxtChunk = makeLtxtChunk(pintId, pointsLenght[i], 0, 0, 0, 0, nullptr);
            ltxtChunks[i] = *ltxtChunk;
            free(ltxtChunk);

            LablChunk* lablChunk = makeLablChunk(pintId, pointsLabels[i]);
    ...
    return makeWaveFile(headerChunk, formatChunk, dataChunk, cueChunk, ltxtChunks,
        ltxtChunksCount, lablChunks, lablChunksCount);
}
...
void saveWaveFile(WaveFile* waveFile, const std::string& filePath)
{
    ...
    if (waveFile->formatChunk != nullptr) {
        ...
        if (fwrite(waveFile->formatChunk, sizeof(FormatChunk), 1, waveFile->file) < 1) {
            LOG_CRITICAL()
                << "[saveWaveFile] Error writing format chunk to output file";
            return;
        }
    }
    if (waveFile->dataChunk != nullptr) {
        ...
        if (fwrite(waveFile->dataChunk->chunkID, sizeof(char), 4, waveFile->file) < 1) {
            LOG_CRITICAL()
                << "[saveWaveFile] Error writing data chunk (chunkID) to output file";
            return;
        }
        if (fwrite(waveFile->dataChunk->chunkDataSize, sizeof(char), 4,
                waveFile->file)
            < 1) {
            LOG_CRITICAL() << "[saveWaveFile] Error writing data chunk "
                              "(chunkDataSize) to output file";
            return;
        }
        if (fwrite(waveFile->dataChunk->waveformData, dataChunkSize, 1,
                waveFile->file)
            < 1) {
            LOG_CRITICAL() << "[saveWaveFile] Size data chunk (waveformData) "
                           << dataChunkSize;
            LOG_CRITICAL() << "[saveWaveFile] Error writing data chunk "
                              "(waveformData) to output file";
            return;
        }
        if (dataChunkSize % 2 != 0) {
            if (fwrite("\0", sizeof(char), 1, waveFile->file) < 1) {
                LOG_CRITICAL()
                    << "[saveWaveFile] Error writing padding character to output file";
                return;
            }
        }
    }
    if (waveFile->cueChunk != nullptr) {
        ...
        LOG_DEBUG() << "[saveWaveFile] Write CUE chunkID " << cue_chunk->chunkID[0]
                    << cue_chunk->chunkID[1] << cue_chunk->chunkID[2]
                    << cue_chunk->chunkID[3];
        if (fwrite(cue_chunk->chunkID, sizeof(char), 4, waveFile->file) < 1) {
            LOG_CRITICAL()
                << "[saveWaveFile] Error writing cue chunk (chunkID) to output file";
            return;
        }

        LOG_DEBUG() << "[saveWaveFile] Write CUE chunkDataSize "
                    << INT32(cue_chunk->chunkDataSize);
        if (fwrite(cue_chunk->chunkDataSize, sizeof(char), 4, waveFile->file) < 1) {
            LOG_CRITICAL() << "[saveWaveFile] Error writing cue chunk "
                              "(chunkDataSize) to output file";
            return;
        }

        LOG_DEBUG() << "[saveWaveFile] Write CUE cuePointsCount "
                    << INT32(cue_chunk->cuePointsCount);
        if (fwrite(cue_chunk->cuePointsCount, sizeof(char), 4, waveFile->file) < 1) {
            LOG_CRITICAL() << "[saveWaveFile] Error writing cue chunk "
                              "(cuePointsCount) to output file";
            return;
        }
        ...
        for (uint32_t i = 0; i < cuePointsCount; i++) {
            ...
            LOG_DEBUG() << "[saveWaveFile] Write CUE POINT cuePointID "
                        << INT32(cue_point->cuePointID);
            if (fwrite(cue_point->cuePointID, sizeof(char), 4, waveFile->file) < 1) {
                LOG_CRITICAL() << "[saveWaveFile] Error writing cue point (cuePointID) "
                                  "to output file";
                return;
            }

            LOG_DEBUG() << "[saveWaveFile] Write CUE POINT playOrderPosition "
                        << INT32(cue_point->playOrderPosition);
            if (fwrite(cue_point->playOrderPosition, sizeof(char), 4,
                    waveFile->file)
                < 1) {
                LOG_CRITICAL() << "[saveWaveFile] Error writing cue point "
                                  "(playOrderPosition) to output file";
                return;
            }

            LOG_DEBUG() << "[saveWaveFile] Write CUE POINT dataChunkID "
                        << cue_point->dataChunkID[0] << cue_point->dataChunkID[1]
                        << cue_point->dataChunkID[2] << cue_point->dataChunkID[3];
            if (fwrite(cue_point->dataChunkID, sizeof(char), 4, waveFile->file) < 1) {
                LOG_CRITICAL() << "[saveWaveFile] Error writing cue point "
                                  "(dataChunkID) to output file";
                return;
            }

            LOG_DEBUG() << "[saveWaveFile] Write CUE POINT chunkStart "
                        << INT32(cue_point->chunkStart);
            if (fwrite(cue_point->chunkStart, sizeof(char), 4, waveFile->file) < 1) {
                LOG_CRITICAL() << "[saveWaveFile] Error writing cue point (chunkStart) "
                                  "to output file";
                return;
            }

            LOG_DEBUG() << "[saveWaveFile] Write CUE POINT blockStart "
                        << INT32(cue_point->blockStart);
            if (fwrite(cue_point->blockStart, sizeof(char), 4, waveFile->file) < 1) {
                LOG_CRITICAL() << "[saveWaveFile] Error writing cue point (blockStart) "
                                  "to output file";
                return;
            }

            LOG_DEBUG() << "[saveWaveFile] Write CUE POINT frameOffset "
                        << INT32(cue_point->frameOffset);
            if (fwrite(cue_point->frameOffset, sizeof(char), 4, waveFile->file) < 1) {
                LOG_CRITICAL() << "[saveWaveFile] Error writing cue point "
                                  "(frameOffset) to output file";
                return;
            }
        }
    }
    if (waveFile->listChunks != nullptr) {
        ...
        for (int listChunkIndex = 0; listChunkIndex < waveFile->listCount;
             listChunkIndex++) {
            ListChunk* list_chunk = &waveFile->listChunks[listChunkIndex];

            if (list_chunk->lablCount == 0 && list_chunk->ltxtCount == 0)
                continue;

            LOG_DEBUG() << "[saveWaveFile] Write LIST chunkID "
                        << list_chunk->chunkID[0] << list_chunk->chunkID[1]
                        << list_chunk->chunkID[2] << list_chunk->chunkID[3];
            if (fwrite(list_chunk->chunkID, sizeof(char), 4, waveFile->file) < 1) {
                LOG_CRITICAL() << "[saveWaveFile] Error writing list chunk (chunkID) "
                                  "to output file";
                return;
            }

            LOG_DEBUG() << "[saveWaveFile] Write LIST chunkDataSize "
                        << INT32(list_chunk->chunkDataSize);
            if (fwrite(list_chunk->chunkDataSize, sizeof(char), 4, waveFile->file) < 1) {
                LOG_CRITICAL() << "[saveWaveFile] Error writing list chunk "
                                  "(chunkDataSize) to output file";
                return;
            }

            LOG_DEBUG() << "[saveWaveFile] Write LIST typeID "
                        << list_chunk->typeID[0] << list_chunk->typeID[1]
                        << list_chunk->typeID[2] << list_chunk->typeID[3];
            if (fwrite(list_chunk->typeID, sizeof(char), 4, waveFile->file) < 1) {
                LOG_CRITICAL() << "[saveWaveFile] Error writing list chunk (typeID) to "
                                  "output file";
                return;
            }
            ...
            for (int i = 0; i < list_chunk->ltxtCount; i++) {
                ...
                LOG_DEBUG() << "[saveWaveFile] Write LTXT chunkID "
                            << ltxt_chank->chunkID[0] << ltxt_chank->chunkID[1]
                            << ltxt_chank->chunkID[2] << ltxt_chank->chunkID[3];
                if (fwrite(ltxt_chank->chunkID, sizeof(char), 4, waveFile->file) < 1) {
                    LOG_CRITICAL() << "[saveWaveFile] Error writing LTXT chunk (chunkID) "
                                      "to output file";
                    return;
                }

                LOG_DEBUG() << "[saveWaveFile] Write LTXT chunkDataSize "
                            << INT32(ltxt_chank->chunkDataSize);
                if (fwrite(ltxt_chank->chunkDataSize, sizeof(char), 4, waveFile->file) < 1) {
                    LOG_CRITICAL() << "[saveWaveFile] Error writing LTXT chunk "
                                      "(chunkDataSize) to output file";
                    return;
                }

                LOG_DEBUG() << "[saveWaveFile] Write LTXT cuePointID "
                            << INT32(ltxt_chank->cuePointID);
                if (fwrite(ltxt_chank->cuePointID, sizeof(char), 4, waveFile->file) < 1) {
                    LOG_CRITICAL() << "[saveWaveFile] Error writing LTXT chunk "
                                      "(cuePointID) to output file";
                    return;
                }

                LOG_DEBUG() << "[saveWaveFile] Write LTXT sampleLength "
                            << INT32(ltxt_chank->sampleLength);
                if (fwrite(ltxt_chank->sampleLength, sizeof(char), 4, waveFile->file) < 1) {
                    LOG_CRITICAL() << "[saveWaveFile] Error writing LTXT chunk "
                                      "(sampleLength) to output file";
                    return;
                }

                LOG_DEBUG() << "[saveWaveFile] Write LTXT purposeID "
                            << ltxt_chank->purposeID[0] << ltxt_chank->purposeID[1]
                            << ltxt_chank->purposeID[2] << ltxt_chank->purposeID[3];
                if (fwrite(ltxt_chank->purposeID, sizeof(char), 4, waveFile->file) < 1) {
                    LOG_CRITICAL() << "[saveWaveFile] Error writing LTXT chunk "
                                      "(purposeID) to output file";
                    return;
                }

                LOG_DEBUG() << "[saveWaveFile] Write LTXT country "
                            << INT16(ltxt_chank->country);
                if (fwrite(ltxt_chank->country, sizeof(char), 2, waveFile->file) < 1) {
                    LOG_CRITICAL() << "[saveWaveFile] Error writing LTXT chunk (country) "
                                      "to output file";
                    return;
                }

                LOG_DEBUG() << "[saveWaveFile] Write LTXT language "
                            << INT16(ltxt_chank->language);
                if (fwrite(ltxt_chank->language, sizeof(char), 2, waveFile->file) < 1) {
                    LOG_CRITICAL() << "[saveWaveFile] Error writing LTXT chunk "
                                      "(language) to output file";
                    return;
                }

                LOG_DEBUG() << "[saveWaveFile] Write LTXT dialect "
                            << INT16(ltxt_chank->dialect);
                if (fwrite(ltxt_chank->dialect, sizeof(char), 2, waveFile->file) < 1) {
                    LOG_CRITICAL() << "[saveWaveFile] Error writing LTXT chunk (dialect) "
                                      "to output file";
                    return;
                }

                LOG_DEBUG() << "[saveWaveFile] Write LTXT codePage "
                            << INT16(ltxt_chank->codePage);
                if (fwrite(ltxt_chank->codePage, sizeof(char), 2, waveFile->file) < 1) {
                    LOG_CRITICAL() << "[saveWaveFile] Error writing LTXT chunk "
                                      "(codePage) to output file";
                    return;
                }
                ...
                if (ltxt_chank->text) {
                    ...
                    if (fwrite(ltxt_chank->text, sizeof(char), size, waveFile->file) < 1) {
                        LOG_CRITICAL() << "[saveWaveFile] Error writing LTXT chunk (text) "
                                          "to output file";
                        return;
                    }
                } else {
            ...
            for (int i = 0; i < list_chunk->lablCount; i++) {
                ...
                LOG_DEBUG() << "[saveWaveFile] Write LABL chunkID "
                            << labl_chank->chunkID[0] << labl_chank->chunkID[1]
                            << labl_chank->chunkID[2] << labl_chank->chunkID[3];
                if (fwrite(labl_chank->chunkID, sizeof(char), 4, waveFile->file) < 1) {
                    LOG_CRITICAL() << "[saveWaveFile] Error writing LABL chunk (chunkID) "
                                      "to output file";
                    return;
                }

                LOG_DEBUG() << "[saveWaveFile] Write LABL chunkDataSize "
                            << INT32(labl_chank->chunkDataSize);
                if (fwrite(labl_chank->chunkDataSize, sizeof(char), 4, waveFile->file) < 1) {
                    LOG_CRITICAL() << "[saveWaveFile] Error writing LABL chunk "
                                      "(chunkDataSize) to output file";
                    return;
                }

                LOG_DEBUG() << "[saveWaveFile] Write LABL cuePointID "
                ...
                if (labl_chank->text) {
                    ...
                    if (fwrite(labl_chank->text, sizeof(char), size, waveFile->file) < 1) {
                        LOG_CRITICAL() << "[saveWaveFile] Error writing LABL chunk (text) "
                                          "to output file";
                        return;
                    }
                } else {
...
std::vector<double> waveformDataToVector(void* data, uint32_t byteSize,
    uint16_t bitDepth)
{
    LOG_DEBUG() << "Start: waveformDataToVector - byteSize=" << byteSize
                << ", bitDepth=" << bitDepth;
    ...
    LOG_DEBUG() << "Finish: waveformDataToVector - samples.size="
                << samples.size();
    return samples;
}
