...
)
{
    ...
    for (int i = 1; i < cuePoints.size() - 1; ++i) {
        ...
        if (cuePoint.type == CuePointType::NUCLEUS) {
            ...
            if (prevCuePoint.type == CuePointType::NUCLEUS && nextCuePoint.type == CuePointType::NUCLEUS) {
                ...
                LOG_DEBUG() << "  Splitting cue point ID " << currentCuePoint.id << " into POST_NUCLEUS and POST_NUCLEUS";
                LOG_DEBUG() << "    Original: position=" << currentCuePoint.position << ", length=" << currentCuePoint.length;
                LOG_DEBUG() << "    POST_NUCLEUS: position=" << preCuePoint.position << ", length=" << preCuePoint.length;
                ...
                LOG_DEBUG() << "    PRE_NUCLEUS: position=" << postCuePoint.position << ", length=" << postCuePoint.length;

                processedCuePoints.push_back(preCuePoint);
                processedCuePoints.push_back(postCuePoint);
            } else {
                // If the PRE_NUCLEUS or POST_NUCLEUS is not between two NUCLEUS, we keep it as is
                processedCuePoints.push_back(cuePoint);
            }
        } else {
    ...
    // Now we should extract segments for each cue point and interpolate them to the target length, then concatenate to form the final UMP vector.
    // For PRE_NUCLEUS and POST_NUCLEUS we should interpalate to make smooth transition between segments.
    int cuePointIndex = 0;
    for (const auto& cuePoint : processedCuePoints) {
        ...
        if (startFrame < endFrame) {
            // if cuePoint is autogenerated, just fill the segment with zeros to avoid introducing noise from the pitch data
            if (cuePoint.label == "_p" || cuePoint.label == "_t") {
                LOG_DEBUG() << "    Autogenerated cue point, filling segment with zeros";
                segment.resize(endFrame - startFrame, 0.0);
                // Replace cue point label removing the _p or _t suffix to indicate that it has been processed
                processedCuePoints[cuePointIndex].label = cuePoint.label.substr(0, cuePoint.label.size() - 2);
            } else {
        ...
        LOG_DEBUG() << "    Interpolated segment size: " << interpolatedSegment.size() << " samples";
